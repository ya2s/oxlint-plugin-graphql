...
- **スキーマが設定されていない場合**（graphql-config が無い、または `schema` が未指定）:
  例外を投げず、`services.schema` を `null` としてそのまま通す。graphql-eslint の
  `parseForESLint` は例外を投げずに成功し、スキーマの要求はルール実行時に
  `requireGraphQLSchema` / `requireGraphQLOperations` が「そのルールが実際に必要とする場合だけ」
  行うため。スキーマ非依存ルール（`no-anonymous-operations` など）は graphql-config が
  一切無いプロジェクトでも本家では動作するので、パース層で一律に禁止してはならない。
  （2026-08-27 実測により訂正。当初この節は「ロード失敗は例外」と書いていたが、
  「スキーマ未設定」と「設定されたスキーマのロード失敗」を混同していた。）
- **設定されたスキーマのロード失敗**（`schema` の指すファイルが存在しない等）: 例外を投げる。
  設定ミスであり fail fast が正しい。本家も同じ挙動で、位置情報を持たない素の `Error` を
  投げるため、構文エラー（`lineNumber` を持つ）との判別がそのまま使える。
